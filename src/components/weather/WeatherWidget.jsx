...
export default function WeatherWidget({ location, latitude, longitude }) {
  ...
  return (
    ...
    >
      <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold text-[#7C3020] md:text-base">
      ...
      <div className="mb-3 rounded-xl border border-brand-100/60 bg-white/58 p-3 backdrop-blur-sm">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div>
              <div className="text-2xl font-bold leading-none text-[#7C3020]">
                {Math.round(cur.temperature_2m)}°C
              </div>
              <div className="text-sm text-[#C07820]">{curLabel}</div>
            </div>
          </div>
          <div className="shrink-0 space-y-1 text-right">
            {cur.wind_speed_10m != null && (
              <div className="flex items-center justify-end gap-1 text-xs text-[#C07820]">
      ...
      {weather.daily?.time?.length >= 3 && (
        <div>
          <h3 className="mb-2 text-xs font-semibold text-[#7C3020]">3-Tage-Vorhersage</h3>
            ...
            {weather.daily.time.slice(0, 3).map((dateStr, i) => {
              ...
              return (
                <div key={dateStr} className="rounded-xl border border-brand-100/60 bg-white/55 p-2 text-center backdrop-blur-sm">
                  ...
                  <div className="mt-1 truncate text-xs text-[#C07820]">{label}</div>
                </div>
              );
            })}
          </div>
      ...
      <p className="mt-3 text-right text-[10px] text-[#C07820]/70">
